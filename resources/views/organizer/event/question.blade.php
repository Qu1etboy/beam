...
@section('title', $event->event_name . "'s " . 'Registrant Questions - Beam Organizer')
      ...
      <div class="flex justify-between border rounded-md p-3 mb-3 gap-3" x-data="{ editing: false}">
        ...
        <div class="flex items-start space-x-2 mt-1">
          ...
          <form action="{{ route('question.destroy', ['question' => $question, 'organizer' => $organizer, 'event' => $event]) }}" method="POST">
            @csrf
            @method('delete')
            <button type="submit" onclick="return confirm('Are you sure you want to delete this question?');" class="text-red-600 flex items-center gap-1 hover:bg-gray-100 rounded-md duration-300 p-1">
