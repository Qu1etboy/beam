...
<div class="w-full h-screen grid grid-cols-1 md:grid-cols-2">
  ...
  <div class="w-full px-16 flex flex-col justify-center">
    ...
    <form action="{{ route('organizer.store-organization') }}" method="POST">
      @csrf
      <div class="mb-6">
        <label for="organizer_name" class="block mb-2 text-sm font-medium text-gray-900">Organization name</label>
        <input type="text" id="organizer_name" name="organizer_name" placeholder="Enter your organization name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
      </div>
