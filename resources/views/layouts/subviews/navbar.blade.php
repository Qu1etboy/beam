  ...
  <nav class="container mx-auto flex justify-between items-center">
    <div class="flex gap-5">
      ...
      <form>
        ...
        <div class="relative w-full">
          ...
          <input type="text" id="simple-search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5" placeholder="Search event name" required>
        </div>
      </form>
    </div>
    ...
    @auth
        <div class="flex items-center gap-3">
          <a href="{{ route('organizer.home') }}" class="hover:text-purple-500 duration-300">Create event</a>        
          <div>
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            {{-- use default Avatar --}}
                            <img src="{{ Auth::user()->avatar }}" alt="user avatar" class="rounded-full object-cover h-10 w-10">
